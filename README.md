...
- 📝 **历史记录**：自动保存计算记录，支持搜索、排序、导出
- 🏷️ **标题功能**：为每次计算添加可识别标题，支持重命名
- 🌓 **主题切换**：支持深色/浅色主题，自动保存偏好
- 🔢 **数字归一化**：自动统一中文/英文/阿拉伯数字表示
- 📥 **数据导出**：支持导出历史记录为 CSV
- 🌐 **局域网共享**：支持多设备同时访问
- 📱 **响应式设计**：适配不同屏幕尺寸
...
- 英文转小写
- 去除特殊符号
- **数字归一化**：将中文/英文数字统一转换为阿拉伯数字
  - 例如："十" → "10"，"ten" → "10"
  - 这样 "10"、"十"、"ten" 会被视为相同，提高准确率
...
- 🌓 **主题切换**：支持深色/浅色主题切换
  ...
  - 支持跟随系统主题
- ⚡ **性能优化**：长文本处理性能大幅提升
  - 2000字中文文本处理时间从数百毫秒降至约1.5ms
  - 优化中文数字归一化算法

### v1.2.5 (2026-04-17)
- 🔤 **英文预处理优化**：修复标点符号导致单词粘连问题
  - 将标点符号替换为空格而非直接删除
  - 例如："hello,world" → "hello world"
  - 提高英文 WER 计算准确率

### v1.2.4 (2026-04-17)
- 📥 **历史记录导出**：支持将历史记录导出为 CSV 文件
  - 支持导出全部记录
  - 支持导出当前筛选结果（按搜索/语言过滤后）
  - CSV 格式，UTF-8 编码，Excel 可直接打开
...
### v1.2.2 (2026-04-14)
- 🔢 **数字归一化**：支持中文/英文数字转换为阿拉伯数字
  - 中文：十、二十一、一百 → 10、21、100
  - 英文：ten、twenty → 10、20
  - 提高数字识别准确率
